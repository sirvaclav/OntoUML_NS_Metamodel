...
@ExpanderTest
class QualityExpanderTest {

  @TestModel(select = "ontoUml::AgeQuality")
  Spec baseModel() {
    return component("ontoUml",
        dataElement("Age"),
        kind("PersonKind"),
        objectType(quality("AgeQuality",
            dataRef("dataElement", "ontoUml::Age"),
            dataRef("qualityExistentialDependency", "ontoUml::PersonKind")
        ))
    );
  }
  ...
  void testArtifactPath(TestExpansion<ObjectTypeComposite> expansion) {
    expansion
        .assertThatExpander()
        .hasArtifactPath("C:/MODULE_ROOT/shared/gen/common/src/AgeQuality.java");
  }
