...
@ExpanderTest
class QuantityExpanderTest {

  @TestModel(select = "ontoUml::WineQuantity")
  Spec baseModel() {
    return component("ontoUml",
        dataElement("Wine"),
        kind("FlaskKind"),
        quantity("WineSpritzerQuantity"),
        objectType(quantity("WineQuantity",
            dataRef("dataElement", "ontoUml::Wine"),
            dataRef("containment", "ontoUml::FlaskKind"),
            dataRef("subQuantityOf", "ontoUml::WineSpritzerQuantity")
        ))
    );
  }
  ...
  void testArtifactPath(TestExpansion<ObjectTypeComposite> expansion) {
    expansion
        .assertThatExpander()
        .hasArtifactPath("C:/MODULE_ROOT/shared/gen/common/src/WineQuantity.java");
  }
